 ...
 *    2. 每周主题挑战   —— 营期 4 周，每周一个主题（启动/晨型人/多面手/冲刺）。
 ...
 * ============================================================================
 */
import { format, startOfWeek, endOfWeek, addDays, differenceInCalendarDays } from 'date-fns';
...
export function computeWeightMilestones(
  ...
  if (!startWeight || weightRecords.length === 0) {
    return [
      { id: 'm3', label: '减重 3%', threshold: 0.03, achieved: false, progress: 0, reward: '一档礼品' },
      { id: 'm5', label: '减重 5%', threshold: 0.05, achieved: false, progress: 0, reward: '二档礼品' },
    ];
  }
 ...
 * 获取当前是营期第几周（1~4）
 *
 * 【规则】以该学员【首次打卡日】为营期第 1 天（而非企业统一开营日——
 *   中途加入的学员按自己的进度走）；第 N 周 = floor((今天-首日)/7)+1，
 *   超过第 4 周按第 4 周计（min 4）。
 *
 * 【注意】若后端有企业统一开营日配置，应改用该日期替换"首次打卡日"，
 *   否则同一批学员的挑战周进度会不一致。当前 MVP 用首次打卡日。
 */
export function getCampWeekIndex(
  ...
  dietRecords.forEach((r) => { if (!userId || !r.studentId || r.studentId === userId) allDates.add(r.date.substring(0, 10)); });
  exerciseRecords.forEach((r) => { if (!userId || !r.studentId || r.studentId === userId) allDates.add(r.date.substring(0, 10)); });
  weightRecords.forEach((r) => { if (!userId || !r.studentId || r.studentId === userId) allDates.add(r.date.substring(0, 10)); });

  if (allDates.size === 0) return 1;
  const start = new Date(Array.from(allDates).sort()[0]);
  const today = new Date();
  return Math.min(Math.floor(differenceInCalendarDays(today, start) / 7) + 1, 4);
}

/**
 * 计算每周主题挑战进度
 *
 * 【规则】营期 4 周各一个主题，周区间 = 以首次打卡日所在自然周（周一为起点）顺延：
...
export function computeWeeklyChallenges(
  ...
  const currentWeek = getCampWeekIndex(dietRecords, exerciseRecords, weightRecords, userId);

  // 获取某周的记录
  const getWeekRecords = (weekIdx: number) => {
    const allDates = new Set<string>();
    dietRecords.forEach((r) => { if (!userId || !r.studentId || r.studentId === userId) allDates.add(r.date.substring(0, 10)); });
    if (allDates.size === 0) return { diets: [], exercises: [], weights: [] };
    const campStart = new Date(Array.from(allDates).sort()[0]);
    const ws = addDays(startOfWeek(campStart, { weekStartsOn: 1 }), (weekIdx - 1) * 7);
    ...
    return { diets: filter(dietRecords), exercises: filter(exerciseRecords), weights: filter(weightRecords) };
  };
  ...
  // 第1周：启动周 - 完成5天全部打卡
  {
    const { diets, exercises, weights } = getWeekRecords(1);
    const allDates = new Set<string>();
    ...
    const done = completeDays >= 5;
    challenges.push({
      id: 'w1', weekIndex: 1, title: '启动周', icon: '🚀',
      description: '完成 5 天全部打卡（三餐+运动+体重）',
      isCurrent: currentWeek === 1, completed: done,
      progressText: `${completeDays}/5 天`,
      progress: Math.min(completeDays / 5, 1),
      status: done ? 'completed' : currentWeek === 1 ? 'active' : currentWeek > 1 ? 'missed' : 'locked',
    });
  }

  // 第2周：晨型人周 - 3次9点前运动
  {
    const { exercises } = getWeekRecords(2);
    const earlyCount = exercises.filter((r) => {
      const t = r.date.split(' ')[1] || '';
      return t >= '05:00' && t < '09:00';
    }).length;
    const done = earlyCount >= 3;
    challenges.push({
      id: 'w2', weekIndex: 2, title: '晨型人周', icon: '🌅',
      description: '3 次在早上 9 点前完成运动打卡',
      isCurrent: currentWeek === 2, completed: done,
      progressText: `${earlyCount}/3 次`,
      progress: Math.min(earlyCount / 3, 1),
      status: done ? 'completed' : currentWeek === 2 ? 'active' : currentWeek > 2 ? 'missed' : 'locked',
    });
  }

  // 第3周：多面手周 - 尝试3种运动类型
  {
    const { exercises } = getWeekRecords(3);
    const types = new Set(exercises.map((r) => r.type));
    const done = types.size >= 3;
    challenges.push({
      id: 'w3', weekIndex: 3, title: '多面手周', icon: '🎽',
      description: '尝试 3 种不同类型的运动',
      isCurrent: currentWeek === 3, completed: done,
      progressText: `${types.size}/3 种`,
      progress: Math.min(types.size / 3, 1),
      status: done ? 'completed' : currentWeek === 3 ? 'active' : currentWeek > 3 ? 'missed' : 'locked',
    });
  }

  // 第4周：冲刺周 - 完成6天全部打卡
  {
    const { diets, exercises, weights } = getWeekRecords(4);
    const allDates = new Set<string>();
    diets.forEach((r) => allDates.add(r.date.substring(0, 10)));
    exercises.forEach((r) => allDates.add(r.date.substring(0, 10)));
    weights.forEach((r) => allDates.add(r.date.substring(0, 10)));
    ...
    const done = completeDays >= 6;
    challenges.push({
      id: 'w4', weekIndex: 4, title: '冲刺周', icon: '🏁',
      description: '完成 6 天全部打卡，为结营冲刺',
      isCurrent: currentWeek === 4, completed: done,
      progressText: `${completeDays}/6 天`,
      progress: Math.min(completeDays / 6, 1),
      status: done ? 'completed' : currentWeek === 4 ? 'active' : currentWeek > 4 ? 'missed' : 'locked',
    });
  }
...
export function computeLuckyDraw(
  ...
  const completionRate = campDays > 0 ? completeDays / campDays : 0;
